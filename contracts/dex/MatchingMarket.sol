...
contract MatchingMarket is MatchingEvents, ExpiringMarket, DSNote {
    ...
    modifier isWhitelist(ERC20 buy_gem, ERC20 pay_gem) {
        require(_menu[keccak256(buy_gem, pay_gem)] || _menu[keccak256(pay_gem, buy_gem)], "token pair is not enabled");
        _;
    }
    ...
    function take(bytes32 id, uint128 maxTakeAmount) public {
        require(buy(uint256(id), maxTakeAmount), "MatchingMarket take fn - buy failed");
    }
    ...
    function offer(
        ...
        returns (uint)
    {
        require(_dust[pay_gem] <= pay_amt, "MM offer fn - pay_amt should be >= _dust amount");
        ...
        returns (bool success)
    {
        if (matchingEnabled) {
            if (isOfferSorted(id)) {
                require(_unsort(id), "cancel-need to remove offer from the sorted list");
            } else {
                require(_hide(id), "cancel-need to hide offer from the unsorted order book");
            }
        }
    ...
    returns (bool)
    {
        require(!isActive(id) && _rank[id].delb != 0 && _rank[id].delb < block.number - 10, "can't delete _rank ");
        delete _rank[id];
    ...
    returns (bool)
    {
        require(!isTokenPairWhitelisted(baseToken, quoteToken), "make sure token pair is not whitelisted");
        require(address(baseToken) != 0x0 && address(quoteToken) != 0x0, "make sure baseToken and quoteTken are not equal to 0x0");
    ...
    returns (bool)
    {
        require(isTokenPairWhitelisted(baseToken, quoteToken), "make sure token pair is whitelisted");
        ...
        returns (uint fill_amt)
    {
        ...
        while (pay_amt > 0) {                           //while there is amount to sell
            ...
            require(offerId != 0, "sellAllAmount-make sure there is at least one offer");                      //Fails if there are not more offers
        ...
        require(fill_amt >= min_fill_amount, "sellAllAmount-make sure fill_amt is >= min_fill_amt");
    }
        ...
        returns (uint fill_amt)
    {
        ...
        while (buy_amt > 0) {                           //Meanwhile there is amount to buy
            offerId = getBestOffer(buy_gem, pay_gem);   //Get the best offer for the token pair
            require(offerId != 0, "buyAllAmount-make sure there is at least one offer");
        ...
        require(fill_amt <= max_fill_amount, "buyAllAmount-make sure fill_amt is <= to min_fill_amt");
    }
    ...
    function getBuyAmount(ERC20 buy_gem, ERC20 pay_gem, uint pay_amt) public constant returns (uint fill_amt) {
        ...
        while (pay_amt > offers[offerId].buy_amt) {
            ...
            if (pay_amt > 0) {                                  //If we still need more offers
                ...
                require(offerId != 0, "getBuyAmount-make sure there is at least one offer");                          //Fails if there are not enough offers to complete
            }
        }
    ...
    function getPayAmount(ERC20 pay_gem, ERC20 buy_gem, uint buy_amt) public constant returns (uint fill_amt) {
        ...
        while (buy_amt > offers[offerId].pay_amt) {
            ...
            if (buy_amt > 0) {                                  //If we still need more offers
                offerId = getWorseOffer(offerId);               //We look for the next best offer
                require(offerId != 0, "getPayAmount-make sure there is at least one offer");                          //Fails if there are not enough offers to complete
            }
        }
    ...
    function _offeru(
        ...
        returns (uint id)
    {
        require(_dust[pay_gem] <= pay_amt, "_offeru-make sure pay_amt is >= to _dust amt");
        ...
        returns (bool)
    {
        address buy_gem = address(offers[id].buy_gem);
        address pay_gem = address(offers[id].pay_gem);
        require(_span[pay_gem][buy_gem] > 0, "_unsort-make sure span[pay_gem][buy_gem] amount is > 0 ");

        require(_rank[id].delb == 0 &&                    //assert id is in the sorted list
                 isOfferSorted(id), "_unsort-make sure assert id is in the sorted list");

        if (id != _best[pay_gem][buy_gem]) {              // offers[id] is not the highest offer
            require(_rank[_rank[id].next].prev == id, "_unsort-offers[id] is not the highest offer");
        ...
        if (_rank[id].prev != 0) {                        //offers[id] is not the lowest offer
            require(_rank[_rank[id].prev].next == id, "_unsort-offers[id] is not the lowest offer");
        ...
        returns (bool)
    {
        ...
        require(!isOfferSorted(id), "_hide-make sure offer id is not in sorted offers list");    //make sure offer id is not in sorted offers list
